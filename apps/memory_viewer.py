...
class MemoryViewer:
    ...
    def update(self):
        ...
        pname = {p.pid: p.app.__class__.__name__[:7] for p in processes}
        ...
        # Grid cells with flash animation
        for idx in range(256):
            ...
            if pid:
                ...
                if age > 0:
                    t = age / fade_max
                    draw_col = tuple(int(base[k]+(255-base[k])*t) for k in range(3))
                else:
                    draw_col = base
                self.os.draw_rect(cx, cy, C-1, C-1, draw_col)
                self.os.draw_pixel(cx, cy, tuple(min(255,v+50) for v in draw_col))
            else:
                if age > 0:
                    t = age / fade_max
        ...
        frag_col = C_GREEN if frag < 0.3 else (C_AMBER if frag < 0.6 else C_RED)
        draw_progress_bar(self.os, self.OX+26, fy+1,
                          self.COLS*C-26, 4,
                          int(frag*100), 100, frag_col)
        self.os.draw_text(self.OX+self.COLS*C-20, fy,
                          str(int(frag*100))+"%", color=frag_col)
        ...
        for p in processes:
            c  = pmap[p.pid]
            nm = pname[p.pid]
            self.os.draw_rect(lx, ly+3, 6, 6, c)
            bc = tuple(min(255,v+80) for v in c) \
                 if p is sched.foreground_process else C_BORDER_DIM
            self.os.draw_border(lx, ly+3, 6, 6, bc)
            self.os.draw_text(lx+8, ly+4, nm, color=c)
            lx += 8 + len(nm)*6 + 6
            if lx > 148:
                break

        # Allocation timeline bar (very bottom)
        tl_y = 144 - 6
        ...
        for i in range(152):
            idx    = int(i * 256 / 152)
            pv     = memory[idx]
            col_c  = pmap.get(pv, (11, 17, 27)) if pv else (11, 17, 27)
